...
class GenerationConfig(FairseqDataclass):
    ...
    waitk_decoding: bool = field(
        default=False,
        metadata={"help": "if set, uses wait-k deocding"},
    )
    waitk_test_lagging: int = field(
        default=5,
        metadata={"help": "lagging number in wait-k"},
    )
    fixed_segment: int = field(
        default=280,
        metadata={"help": "fixed segment (ms) of speech in wait-k"},
    )
    itst_decoding: bool = field(
        default=False,
        metadata={"help": "if set, uses itst deocding"},
    )
    itst_test_threshold: float = field(
        default=0.4,
        metadata={"help": "test threshold in ITST"},
    )
