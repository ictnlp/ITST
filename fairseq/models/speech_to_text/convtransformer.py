...
class ConvTransformerEncoder(FairseqEncoder):
    ...
    def __init__(self, args):
        ...
        if self.uni_encoder:
            tmp = (
                torch.arange(
                    1, math.ceil(seq_len / self.pre_decision_ratio) + 1, device="cuda"
                )
                .unsqueeze(1)
            ...
            block_mask = (
                torch.arange(0, seq_len, device="cuda")
                .unsqueeze(0)
                .repeat(seq_len, 1)
                .float()
            )
            block_mask = block_mask.masked_fill(
                (block_mask <= (tmp * self.pre_decision_ratio - 1)), 0.0
            )
            self.block_mask = block_mask.masked_fill(
                (block_mask > (tmp * self.pre_decision_ratio - 1)), float("-inf")
            )
