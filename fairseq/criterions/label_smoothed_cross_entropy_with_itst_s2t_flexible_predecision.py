...
def bulid_weight_matrix_s2t(src_len, tgt_len):
    r = src_len / tgt_len
    a = torch.arange(1, src_len + 1, device="cuda").unsqueeze(0)
    b = torch.arange(3, tgt_len + 3, device="cuda").unsqueeze(1) * r
    w = torch.abs(a - b) - (src_len // tgt_len // 2)
    w = (w).max(torch.zeros((1, 1), device="cuda"))
    w = w.unsqueeze(0).unsqueeze(1) / (src_len)
    return w


@register_criterion("label_smoothed_cross_entropy_with_itst_s2t_flexible_predecision")
class LabelSmoothedCrossEntropyCriterionWithITSTS2TFlexiblePredecison(FairseqCriterion):
    def __init__(
        self,
        task,
        sentence_avg,
    ...
    def forward(self, model, sample, update_num=None, reduce=True):
        ...
        info_transport = net_output[1]["info_transport"]
        ...
        loss = ce_loss + info_loss + norm_loss
        ...
        logging_output = {
            ...
            "info_loss": info_loss.data,
            "norm_loss": norm_loss.data,
    ...
    def reduce_metrics(cls, logging_outputs) -> None:
        ...
        ce_loss_sum = sum(log.get("cd_loss", 0) for log in logging_outputs)
        ...
        metrics.log_scalar(
            "info_loss", info_loss_sum / sample_size / math.log(2), sample_size, round=3
        )
        metrics.log_scalar(
            "norm_loss", norm_loss_sum / sample_size / math.log(2), sample_size, round=3
        )
        metrics.log_scalar(
