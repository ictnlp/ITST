...
class SpeechToTextDataset(FairseqDataset):
    ...
    def __init__(
        ...
        self.src_bpe_tokenizer = bpe_tokenizer
        # self.src_bpe_tokenizer = spm.SentencePieceProcessor()
        # self.src_bpe_tokenizer.Load("/path/spm_unigram5000_asr.model")
        # self.src_bpe_tokenizer.Load("/data/zhangshaolei/datasets/mustc_mos/en-de/spm_unigram10000_raw.model")
    ...
    def __getitem__(
        ...
        if self.src_texts is not None:
            ...
            if self.pre_tokenizer is not None:
                text = self.pre_tokenizer.encode(text)
            # transcription = self.src_bpe_tokenizer.EncodeAsPieces(text)
            transcription = self.src_bpe_tokenizer.encode(text)
            # print(transcription)
        return index, source, target, transcription
    ...
    def collater(self, samples: List[Tuple[int, torch.Tensor, torch.Tensor]]) -> Dict:
        ...
        if self.src_texts is not None:
            transcription_lengths = torch.tensor(
                [len(t) if len(t) else 1 for _, _, _, t in samples], dtype=torch.long
            ).index_select(0, order)
            # print(transcription_lengths)
