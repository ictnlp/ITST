...
from fairseq.models.speech_to_text.convtransformer import ConvTransformerEncoder
...
class TransformerUnidirectionalEncoder(ConvTransformerEncoder):
...
class TransformerMonotonicDecoder(TransformerDecoder):
    ...
    def extract_features(
        ...
        src_len = encoder_out["encoder_out"][0].size(0)
        if self.multipath and incremental_state is None and step is None:
            global_wait_k = random.randint(
                1, (src_len // self.args.fixed_pre_decision_ratio) + 1
            )
        else:
            global_wait_k = None
        ...
        p_chooses = [x["p_choose"] for x in attn_list]
        p_choose = torch.cat(p_chooses, dim=1)
        p_choose = p_choose.mean(dim=1)
