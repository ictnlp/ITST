...
class MonotonicAttention(MultiheadAttention):
    ...
    def add_args(parser):
        ...
        parser.add_argument('--attention-eps', type=float, default=1e-4,
    ...
    def monotonic_attention_process_infer(
        ...
        p_choose, _, _, _ = self.p_choose(query, key, None, incremental_state)
    ...
    def monotonic_attention_process_train(
        ...
        if self.waitk_lagging <= 0 or not self.training:
            ...
            # orginal implementation of wait-k policy from Fairseq is too computationally complex, we implement a quick calculation (from Shaolei Zhnag).
...
class WaitKAttention(MonotonicInfiniteLookbackAttention):
    ...
    def add_args(parser):
        ...
        parser.add_argument(
            "--waitk-lagging", type=int, default=5, help="Wait K lagging"
        )
        parser.add_argument(
        ...
        parser.add_argument(
            ...
            action="store_true",
            default=False,
            help="average gate in MoE Wait-k",
        )
