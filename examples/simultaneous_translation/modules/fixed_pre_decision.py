...
from .itst_multihead_attention import InformationTransportAttention
from .moe_multihead_attention import MoEWaitKAttention
    ...
    def create_model(monotonic_attention, klass):
        class FixedStrideMonotonicAttention(monotonic_attention):
            ...
            def insert_zeros(self, x):
                ...
                stride = self.pre_decision_ratio
                weight = F.pad(
                    torch.ones((1, 1, 1), device=x.device), (stride - 1, 0)
                ).type_as(x)
            ...
            def copy_insert(self, x):
                bsz_num_heads, tgt_len, src_len = x.size()
                stride = self.pre_decision_ratio
                x_upsample = x.unsqueeze(3).repeat(1, 1, 1, stride)
                return x_upsample.contiguous().view(bsz_num_heads, tgt_len, -1)
            ...
            def p_choose(
                ...
                p_choose_copy = self.copy_insert(p_choose_pooled)
                ...
                if p_choose_copy.size(-1) < src_len:
                    # Append zeros if the upsampled p_choose is shorter than src_len
                    p_choose_copy = torch.cat(
                        [
                            p_choose_copy,
                            torch.zeros(
                                p_choose_copy.size(0),
                                tgt_len,
                                src_len - p_choose_copy.size(-1),
                                device=p_choose_copy.device,
                            ),
                        ],
                        dim=2,
                    )
                    ...
                    # can be larger than src_len because we used ceil before
                    p_choose_copy = p_choose_copy[:, :, :src_len]
                    p_choose_copy[:, :, -1] = p_choose_pooled[:, :, -1]

                assert list(p_choose_copy.size()) == [
                    batch_size * self.num_heads,
                    tgt_len,
                    src_len,
                ]
                if key_padding_mask_pool is not None:
                    key_padding_mask_pool = key_padding_mask_pool.view(
                        batch_size, self.num_heads, -1
                    )[:, 0, :]

                return p_choose, p_choose_copy, p_choose_pooled, key_padding_mask_pool
...
@register_monotonic_attention("MoE_waitk_fixed_pre_decision")
@fixed_pooling_monotonic_attention(MoEWaitKAttention)
class MoEWaitKAttentionFixedStride:
    pass


@register_monotonic_attention("ITST_fixed_pre_decision")
@fixed_pooling_monotonic_attention(InformationTransportAttention)
class InformationTransportAttentionFixedStride:
    pass
