...
def process(args):
    ...
    for split in MUSTC.SPLITS:
        ...
        for wav, offset, n_frames, sr, src_utt, tgt_utt, speaker_id, utt_id in tqdm(
            dataset
        ):
    ...
    for split in ["tst-COMMON", "dev"]:
        ...
        save_df_to_tsv(df, cur_root / f"{split}_raw_st.tsv")
    # Generate config YAM
    gen_config_yaml_raw(
        cur_root,
        None,
        yaml_filename=f"config_raw_joint.yaml",
    )
