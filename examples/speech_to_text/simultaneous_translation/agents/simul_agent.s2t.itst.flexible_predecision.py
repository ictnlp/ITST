...
class FairseqSimulSTAgent(SpeechAgent):
    ...
    def load_model_vocab(self, args):
        ...
        if args.test_threshold is not None:
            print("test threshold: ", args.test_threshold)
    ...
    def policy(self, states):
        ...
        cum_source_info = outputs["info_transport"][0, :, :, :-1].sum(
            dim=-1, keepdim=False
        )[:, -1]

        if (
            (cum_source_info < self.test_threshold).sum() > 0
            or (cur_tgt_len + 2) * 280 * 16 > cur_src_len
