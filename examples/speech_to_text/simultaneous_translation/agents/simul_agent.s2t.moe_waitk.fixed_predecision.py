# Read/Write policy of MoE wait-k with fixed pre-decision
#
# by Shaolei Zhang
...
class FairseqSimulSTAgent(SpeechAgent):
    ...
    def add_args(parser):
        ...
        parser.add_argument("--test-waitk-lagging", type=int, default=5,
                            help="threshold to start translating.")
    ...
    def load_model_vocab(self, args):
        if self.gpu:
            self.device_id = random.randint(0, torch.cuda.device_count() - 1)

        filename = args.model_path
        if not os.path.exists(filename):
            raise IOError("Model file not found: {}".format(filename))
        arg_overrides = {"waitk_lagging": args.test_waitk_lagging}
